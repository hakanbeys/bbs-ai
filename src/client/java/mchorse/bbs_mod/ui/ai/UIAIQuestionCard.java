...
public class UIAIQuestionCard extends UIElement
{
    ...
    public UIAIQuestionCard(UIAIMessageParser.QuestionCardData data, Consumer<String> onSubmit)
    {
        ...
        if (data.multi)
        {
            ...
            UIButton submit = new UIButton(IKey.raw(data.submitLabel), (b) ->
            {
                ...
                {
                    disableControls();
                    onSubmit.accept(buildReply(data, selected));
                }
            });
            submit.w(1F).h(22);
        ...
        else
        {
            for (String option : data.options)
            {
                ...
                {
                    disableControls();
                    onSubmit.accept(buildReply(data, option));
                });
                button.w(1F).h(22);
    ...
    private String buildReply(UIAIMessageParser.QuestionCardData data, String option)
    {
        return "Selection for \"" + data.title + "\": " + option;
    }

    private String buildReply(UIAIMessageParser.QuestionCardData data, List<String> options)
    {
        return "Selections for \"" + data.title + "\": " + String.join(", ", options);
    }
