...
import mchorse.bbs_mod.ui.ai.AIAnimatorManager;
import mchorse.bbs_mod.ui.ai.UIAIChatPanel;
...
public class UIFilmPanel extends UIDataDashboardPanel<Film> implements IFlightSupported, IUIOrbitKeysHandler, ICursor
{
    ...
    public UIFilmPanel(UIDashboard dashboard)
    {
        ...
        this.openAIAnimator = new UIIcon(Icons.PROCESSOR, (b) ->
        {
            if (this.data != null)
            {
                String filmId = "film_" + this.data.hashCode();
                UIAIChatPanel panel = new UIAIChatPanel(
                    AIAnimatorManager.getOrchestrator(),
                    this.data, filmId,
                    this.undoHandler.getUndoManager()
                );
                UIOverlay.addOverlay(this.getContext(), panel, 0.7F, 0.8F);
            }
        });
        this.openAIAnimator.tooltip(IKey.raw("AI Animator"), Direction.LEFT);

        /* Setup elements */
        this.iconBar.add(this.openHistory, this.toggleHorizontal.marginTop(9), this.openCameraEditor.marginTop(9), this.openReplayEditor, this.openActionEditor, this.openAIAnimator.marginTop(9));
